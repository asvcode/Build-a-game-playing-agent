...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    print("\n{0:^9}{0:^13}".format("Match #", "Opponent") + ''.join(['{0:^13}'.format(x[1].name) for x in enumerate(test_agents)]))
    print("{0:^9}{0:^13} ".format("", "") +  ' '.join(['{0:^5}| {0:^5}'.format("Won", "Lost") for x in enumerate(test_agents)]))

    for idx, agent in enumerate(cpu_agents):
        wins = {key: 0 for (key, value) in test_agents}
        wins[agent.player] = 0

        #print("{0:^9}{0:^13}".format(idx + 1, agent.name), end="", flush=True)
        print("{0:^9}{0:^13}".format(idx + 1, agent.name), end="")
    ...
    print("-" * 74)
    print('{0:^9}{0:^13}'.format("", "Win Rate:") +
        ''.join([
            '{0:^13}'.format(
                "{0:.1f}%".format(100 * total_wins[x[1].player] / total_matches)
